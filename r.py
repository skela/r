...
class AppIconSize(object):
	...
	def get_file_name(self, scale) -> str:
...
class RConfig(object):
	...
	@staticmethod
	def setup_path_to_inkscape(path_inkscape) -> str:
		path = path_inkscape
	...
	@staticmethod
	def setup_path_to_convert(path_convert) -> str:
		path = path_convert
	...
	@staticmethod
	def setup_path_to_svg2pdf(path_svg2pdf) -> str:
		path = path_svg2pdf
...
class R(object):
	...
	def svg2png(self, width, height, png_file, svg_file, options=None, update_lock=True):
		w = width
		h = height
	...
	def svg2pdf(self, svg_file, pdf_file):
		...
		in_file = os.path.abspath(svg_file)
		out_file = os.path.abspath(pdf_file)

		if self.lock.check_for_skippage("svg2pdf", "0", "0", in_file, out_file):
			return None
		...
		self.lock.update("svg2pdf", "0", "0", in_file, out_file)
	...
	def xcf2png(self, width, height, out_file, in_file):
		...
		if self.lock.check_for_skippage("xcf2png", width, height, in_file, out_file):
			return None
		...
		self.lock.update("xcf2png", width, height, in_file, out_file)
	...
	def xcf2pdf(self, width, height, out_file, in_file):
		...
		if self.lock.check_for_skippage("xcf2pdf", width, height, in_file, out_file):
			return None
	...
	def svg2webps(self, width1x, height1x, out_file, in_file):
		...
		if not isinstance(width1x, str):
			w = str(width1x)
		if not isinstance(height1x, str) and h is not None:
			h = str(height1x)
	...
	def svg2android_icons(self, svg_file, target, round=False):
		icon_sizes = [(36, 36, "mipmap-ldpi"), (48, 48, "mipmap-mdpi"), (72, 72, "mipmap-hdpi"), (96, 96, "mipmap-xhdpi"), (144, 144, "mipmap-xxhdpi"), (192, 192, "mipmap-xxxhdpi")]
	...
	def convert(self, in_path_png, out_path_png, actions=None):
		if actions is None:
			cmd = '%s "%s" "%s"' % (self.path_convert, in_path_png, out_path_png)
			os.system(cmd)
